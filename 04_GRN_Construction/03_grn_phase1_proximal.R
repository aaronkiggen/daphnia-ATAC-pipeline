...
HOMER_DEFAULT  <- "homer_out_default_proximal/knownResults.txt"
HOMER_CUSTOM   <- "homer_out_custom_bg_proximal/knownResults.txt"
FIMO_TXT       <- "fimo_proximal/fimo.txt"   # Note: Changed to fimo.txt from previous pipeline
PEAKS_BED      <- "peaks_near_DEG_proximal.bed"   
DEGS_CSV       <- "DEGs.csv"
...
write.csv(homer_enriched, file.path(OUTPUT_DIR, "01_Enriched_Motifs_TFs_proximal.csv"), row.names=FALSE)
...
peak_gene_raw <- read.table("peak_DEG_overlaps_proximal.txt", 
...
write.csv(grn_edges, file.path(OUTPUT_DIR, "01_Base_Motif_Network_Edges_proximal.csv"), row.names=FALSE)
...
write.csv(top_tfs, file.path(OUTPUT_DIR, "01_Candidate_TFs_Summary_proximal.csv"), row.names=FALSE)

cat("\nNetwork edge count:", nrow(grn_edges), "- Hubs saved to top_TF_candidates_proximal.csv\n")
